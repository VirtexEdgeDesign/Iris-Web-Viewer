...
vxMeshPart.prototype.initMaterials = function() {
  ...
  if (material == null) {
    ...
    this.Mesh.logLoadError(
      "Material '" + this.MaterialKey + "' not found!",
      "The referenced material '" +
        this.MaterialKey +
        "' can't be found! This is usually caused by a missing '*.mtl' file."
    );

    log('// Material ' + this.MaterialKey + ' not found');
    ...
    ioMaterials[
      this.MaterialKey.toString()
        .trim()
        .valueOf()
    ] = new vxMaterial(this.MaterialKey);
    ...
    if (textName !== '') {
      if (src == null) {
        ...
        var missing_src =
          'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAIAAAACAAgMAAAC+UIlYAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH4QkcAxoOU8CBVAAAAAlQTFRFqAD//wD/////dFYIKwAAAAFiS0dEAmYLfGQAAAA3SURBVFjD7dShAcAwAMMwP5n/XxktKSubUIhQgNu2qtsG/Ar4AjjXB4A+APoA6AOgD4A+AI/AB1lFqB/c8lphAAAAAElFTkSuQmCC';
        ...
        this.Mesh.logLoadError(
          "Texture '" + textName + "' not found!",
          "The referenced texture '" +
            textName +
            "' can't be found! This is usually caused by a missing texture file."
        );
      } else {
        this.HasTexture = 1;
...
vxMeshPart.prototype.Init = function() {
  ...
  for (var i = 0; i < this.mesh_vertices.length; i += 3) {
    TestPoint.Set(
      this.mesh_vertices[i],
      this.mesh_vertices[i + 1],
      this.mesh_vertices[i + 2]
    );
...
vxMeshPart.prototype.InitialiseBuffers = function() {
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.mesh_vertices),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.vert_noramls),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.vert_uvcoords),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.vert_colours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.highlight_colours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.vert_selcolours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.wire_colours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.edgevertices),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.edge_noramls),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.edge_uvcoords),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.edge_colours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(this.edge_highlight_colours),
    gl.STATIC_DRAW
  );
  ...
  gl.bufferData(
    gl.ELEMENT_ARRAY_BUFFER,
    new Uint16Array(this.EdgeIndices),
    gl.STATIC_DRAW
  );
};
...
vxMeshPart.prototype.DrawSelPreProc = function() {
  if (this.Enabled === true) {
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.meshBuffers.uvCoords);
    gl.vertexAttribPointer(
      shader.attribLocations.txtrCoords,
      2,
      gl.FLOAT,
      false,
      0,
...
vxMeshPart.prototype.Draw = function() {
  if (this.Enabled === true) {
    gl.uniform3fv(shader.uniformLocations.ambientColour, [
      this.AmbientColour.R,
      this.AmbientColour.G,
      this.AmbientColour.B
    ]);

    var doTexture = RenderState == vxRenderState.Textured ? 1 : 0;
    gl.uniform1i(
      shader.uniformLocations.HasTexture,
      this.HasTexture * doTexture
    );
...
vxMeshPart.prototype.AddVertices = function(
  vertices,
  normal,
  texCoord,
  colour,
...
vxMeshPart.prototype.AddFace = function(
  vert1,
  vert2,
  vert3,
  norm1,
