...
function drawScene() {
  if (safeToDraw == true) {
    ...
    gl.readPixels(
      MouseState.x,
      gl.drawingBufferHeight - MouseState.y,
      1,
      1,
      gl.RGBA,
      gl.UNSIGNED_BYTE,
    ...
    if (HoverIndex > 0 && MeshCollectionPart.length > 0) {
      ...
      for (var i = 0; i < MeshCollectionPart.length; i++) {
        ...
        if (
          HoverIndex >= MeshCollectionPart[i].IndexStart &&
          HoverIndex < MeshCollectionPart[i].IndexEnd
        ) {
          //Now Finally Draw the Face
          for (var j = 0; j < 9; j++) {
            HoveredMesh.mesh_vertices[j] =
              MeshCollectionPart[i].mesh_vertices[
                (HoverIndex - 1 - runningtotal) * 9 + j
              ];
            HoveredMesh.vert_noramls[j] =
              MeshCollectionPart[i].vert_noramls[
                (HoverIndex - 1 - runningtotal) * 9 + j
              ];
          }
          HoveredMesh.Model = MeshCollectionPart[i].Model;
          HoveredMesh.Mesh = MeshCollectionPart[i].Mesh;
        ...
        runningtotal =
          runningtotal +
          (MeshCollectionPart[i].IndexEnd - MeshCollectionPart[i].IndexStart);
      }
    } else {
      for (var i = 0; i < 9; i++) HoveredMesh.mesh_vertices[i] = 0;
...
function CreateGrid(gridSize) {
  ...
  for (var i = -gridSize; i < gridSize + 1; i += 5) {
    if (i % 20 === 0) temp_colour = [1, 1, 1, 1];
    else temp_colour = [0.7, 0.7, 0.7, 1];
