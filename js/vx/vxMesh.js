...
vxMesh.prototype.initMaterials = function() {
  for (var i = 0; i < this.MeshParts.length; i++) {
    ...
    this.Materials[this.MeshParts[i].MaterialKey] = this.MeshParts[
      i
    ].getMaterial();
  }
};
...
vxMesh.prototype.Init = function() {
  ...
  for (var i = 0; i < this.MeshParts.length; i++) {
    ...
    if (this.MeshParts[i].MaxPoint.Length() > this.MaxPoint.Length())
      this.MaxPoint.Set(
        this.MeshParts[i].MaxPoint.X,
        this.MeshParts[i].MaxPoint.Y,
        this.MeshParts[i].MaxPoint.Z
      );
  }
...
vxMesh.prototype.AddVertices = function(
  vertices,
  normal,
  texCoord,
  colour,
...
vxMesh.prototype.AddFace = function(
  vert1,
  vert2,
  vert3,
  norm1,
