function io_import_stl(file, InputFileText) {
  ...
  if (isASCII === false) {
    ...
    var mesh = new vxMesh(
      'mesh: ' + file.name.substring(0, file.name.length - 4)
    );
    log("// Loading '" + file.name + "' as a ASCII .stl file...");
...
function io_import_stl_binary(file, InputFileText) {
  log("// Loading '" + file.name + "' as a BINARY .stl file...");
      ...
      for (var j = 0; j < mesh.MeshParts.length; j++) {
        ...
        for (var k = 0; k < meshpart.Indices.length; k += 3) {
          ...
          fileWriter.writeLine(
            'facet normal ' + ni1[0] + ' ' + ni1[1] + ' ' + ni1[2]
          );
          ...
          fileWriter.writeLine(
            'vertex ' +
              meshpart.mesh_vertices[vi1[0] * 3] +
              ' ' +
              meshpart.mesh_vertices[vi1[0] * 3 + 1] +
              ' ' +
              meshpart.mesh_vertices[vi1[0] * 3 + 2]
          );
          fileWriter.writeLine(
            'vertex ' +
              meshpart.mesh_vertices[vi1[1] * 3] +
              ' ' +
              meshpart.mesh_vertices[vi1[1] * 3 + 1] +
              ' ' +
              meshpart.mesh_vertices[vi1[1] * 3 + 2]
          );
          fileWriter.writeLine(
            'vertex ' +
              meshpart.mesh_vertices[vi1[2] * 3] +
              ' ' +
              meshpart.mesh_vertices[vi1[2] * 3 + 1] +
              ' ' +
              meshpart.mesh_vertices[vi1[2] * 3 + 2]
          );
